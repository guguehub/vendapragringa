...
export class CreateUserItem1698463000100 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createForeignKeys('user_items', [
      new TableForeignKey({ name: 'FKUserItemsUser', columnNames: ['user_id'], referencedTableName: 'users', referencedColumnNames: ['id'], onDelete: 'CASCADE' }),
      new TableForeignKey({ name: 'FKUserItemsItem', columnNames: ['item_id'], referencedTableName: 'items', referencedColumnNames: ['id'], onDelete: 'CASCADE' }),
    ]);

    await queryRunner.createIndex(
      'user_items',
      new TableIndex({ name: 'IDX_USER_ITEM_UNIQUE', columnNames: ['user_id', 'item_id'], isUnique: true }),
    );
  }
